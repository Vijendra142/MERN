...
import {orderConfirm, sendotpregister} from './Sendmail'
...
router.post('/verify-otp',async(req:Request,res:Response,next:NextFunction)=>{
    try{
        ...
        if(!user){
            ...
            const verifyotp = await Usemodel.findOne({email:email,otp:otp,status:'inactivate'});

            if(verifyotp){
                await Usemodel.findOneAndUpdate({email:email},{
                    status:'activate'
                })
...
router.post('/resend-otp',async(req:Request,res:Response,next:NextFunction)=>{
    try{
        ...
        const user = await Usemodel.findOne({email:email,status:'inactivate'})

        const otp = registerotp();

        if(user){
           const updateopt = await Usemodel.findOneAndUpdate({email:email},{
                otp:otp
            })
...
router.post('/login',validate(login), async (req: Request, res: Response, next: NextFunction) => {
    try {
        ...
        const users = await Usemodel.findOne({ email: email, password: password,status:'activate' })
        if (!users) {
            return res.status(404).json({
                success: false,
                message: "no user found"
        ...
        for (const order of orders) {
            ...
            if(newOrder){
                ...
                await Cartproduct.deleteMany();
                 ...
                 const products = await ProductSchema.findById(newOrder.product);

                 const users = await Usemodel.findById(userid);
        
               const email = users?.email || 'talariramcharan33@gmail.com';
...
router.get("/cuopncode",async(req:Request,res:Response,next:NextFunction)=>{
    try{
